<template>
    <section class="posts-container">
      <div class="posts-grid">
        <div v-for="video in videos" :key="video.uid" class="post">
            <PrismicLink :field="video" :linkResolver="linkResolver" class="card-link">
            <div class="image-wrapper">
                <PrismicImage :field="video.data.borito" class="base-image" />
            </div>
            <div class="text-wrapper">
                <p class="category">{{ video.data.kategoria }} <span class="date">- {{ video.data.datum }}</span></p>
                <PrismicText :field="video.data.cim" wrapper="p" class="title" />
            </div>
        </PrismicLink>
        </div>
      </div>
...
const { data: videos } = useAllPrismicDocumentsByType("video", {
    orderings: [
